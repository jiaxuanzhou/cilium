...
func ResolveIdentityPolicy(rules api.Rules, identityCache identity.IdentityCache, identityToResolve identity.NumericIdentity) *cilium.NetworkPolicy {
	...
	for _, rule := range selectingRules {
		for _, ingressRule := range rule.Ingress {
			if len(ingressRule.FromRequires) > 0 && len(ingressRule.FromEndpoints) == 0 {
				// Do not generate policy for rule that does not have FromEndpoints
				// coupled with FromRequires.
				continue
			}
		...
		for _, egressRule := range rule.Egress {
			if len(egressRule.ToRequires) > 0 && len(egressRule.ToEndpoints) == 0 {
				// Do not generate policy for rule that does not have
				// ToEndpoints coupled with ToRequires.
				continue
			}
